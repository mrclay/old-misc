...
/**
 * WARNING! This is an attempt to port java.util.regex.Matcher. It's probably full of bugs.
 * Don't use it.
 *
...
class Matcher {
    ...
    /**
     * @var Pattern The Pattern object that created this Matcher.
     */
    ...
    /**
     * @var UTF-8 string The original string being matched.
     */
    protected $text;
     ...
     */
    public function __construct(Pattern $parent, StringBuilder $text)
    {
     ...
     * @return Matcher
     */
    public function reset(StringBuilder $input = null)
    {
        if ($input !== null) {
            $this->text = $input->getBytes();
        }
        $this->first = -1;
        $this->last = 0;
        $this->oldLast = -1;
    ...
    {
        return $this->parentPattern->capturingGroupCount();
    }
     ...
     * @return StringBuilder
     */
    public function quoteReplacement(StringBuilder $s)
    {
        return new StringBuilder(preg_quote($s->getBytes(), '/'));
    }
    ...
    public function appendReplacement(StringBuilder $sb, StringBuilder $replacement)
    {
        ...
        while ($cursor < $replacement->length()) {
            ...
            if ($nextChar == '\\') {
                ...
                while (!$done) {
                    ...
                    $nextDigit = $replacement->charAt($cursor) - '0';
        ...
        $sb->append(new StringBuilder(substr($this->text, $this->lastAppendPosition, ($this->first - $this->lastAppendPosition))));
     ...
     * @return StringBuilder
     */
    public function appendTail(StringBuilder $sb)
    {
        $sb->append(new StringBuilder(substr($this->text, $this->lastAppendPosition)));
        return $sb;
    }
     ...
     * @return StringBuilder
     */
    public function replaceAll(StringBuilder $replacement)
    {
        $this->reset();
        $result = $this->find();
        if ($result) {
     ...
     * @return StringBuilder
     */
    public function replaceFirst(StringBuilder $replacement)
    {
        $this->reset();
        if (!$this->find())
            return new StringBuilder($this->text);
    ...
    protected function search($from)
    {
        ...
        if ($this->anchoringBounds) {
            ...
            $result = preg_match($this->parentPattern->getPattern(), substr($this->text, $from), $m, PREG_OFFSET_CAPTURE);
            if ($result) {
            ...
            $result = preg_match($this->parentPattern->getPattern(), $this->text, $m, PREG_OFFSET_CAPTURE, $from);
        }
        if ($result) {
            $this->first = $m[0][1];
